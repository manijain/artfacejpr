...
$(function() {
      ...
      if(confirm("Do you want to delete the wishlist")) {
        $.ajax({
          ...
          success: function(data) {
            if(data == "true") {
              $("#" + wishlist_id).parent().fadeOut(1500)
  ...
  // to render a new partial to create a new wishlist
    $(document).on('click', "#add_new_wishlist" , function(){
       $.ajax({
        url: "/wishlist/add_new_wishlist",
        success: function(data) {
           add_new_wish_toggle()
           jQuery('html, body').animate({ scrollTop: 5000 },700)
        }
      })
    })
